...
public static class WorldTest
{
    ...
    /// Площадки для замера формы мира: все этажи, а не только верхний силуэт.
    /// Допуск по наклону здесь нарочно широкий — тест меряет ярусность и
    /// острова, а не качество высадки. Настоящая раскладка (SpawnLayout) сначала
    /// просит ровные площадки и отпускает допуск, только если их мало.
    static List<Vector2> Spawns(DestructibleTerrain t) => t.CollectSpawnPoints(1f, 1.2f);
